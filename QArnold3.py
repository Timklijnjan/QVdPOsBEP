...
for V in Vsp:
    ...
    for Domg in Domgsp:
        ...
        omg3 = omg2 +0.4545*V # for skewed Arnold tongue use +0.4545*V, normal used +Domg*gamup
...
qsave(Cvals,'CmodvalscircskewedD')

fig, ax = plt.subplots() 
cont0 = ax.contourf(Domgsp,Vspplot,Cvals,levels=100)
ax.plot(posdelt, 2.2725*posdelt, 'k--');
ax.plot(negdelt, -2.2725*negdelt,'k--');
ax.plot(posdeltfull,1.1364*posdeltfull,'k-');
ax.plot(negdeltfull,-1.1364*negdeltfull,'k-');
ax.set_aspect(1)
